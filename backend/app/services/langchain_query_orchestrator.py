"""Main query orchestrator using LangChain."""
...
class LangChainQueryOrchestrator:
    """Coordinates query processing using LangChain pipeline."""
    
    def __init__(self):
        """Initialize services."""
        self.langchain_orch = get_langchain_orchestrator()
    ...
    async def process_query(
        ...
        """
        Process a complete COREP query using LangChain.
        
        Pipeline:
        1. Retrieve relevant regulatory text via LangChain retriever
        2. Generate structured output via LangChain chain
        3. Map to template fields
        4. Validate populated fields
        5. Generate audit trail
        """
        
        logger.info(f"Processing query {query_id} with LangChain: {question[:100]}")
        
        # Step 1: Retrieve relevant documents via LangChain
        retrieved_context = self.langchain_orch.retrieve_documents(question)
        ...
        # Step 3: Process via LangChain chain
        llm_response = await self.langchain_orch.process_scenario(
            question=question,
        ...
        logger.info(f"Query {query_id} completed successfully via LangChain")
        return response
